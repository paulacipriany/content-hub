...
import { Platform, ContentType } from '@/data/types';
...
const XTwitterSvg = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
...
const GoogleBusinessSvg = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M2 7h20l-2 4h-1v9h-2v-9h-4v9H4v-9H3L2 7zm3 4v7h4v-7H5zm1-9h12v4H6V2z"/>
    <path d="M5.5 7l-1 2h1.5l1-2H5.5zm3 0l-1 2H9l1-2H8.5zm3 0l-1 2h1.5l1-2h-1.5zm3 0l-1 2H15l1-2h-1.5zm3 0l-1 2h1.5l1-2h-1.5z" opacity="0.6"/>
  </svg>
);

const BlogSvg = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
...
export const getContentTypesForPlatforms = (platforms: Platform[]): ContentType[] => {
  if (platforms.length === 0) return [];
  const sets = platforms.map(p => new Set(PLATFORM_CONTENT_TYPES[p]));
  const allTypes = new Set<ContentType>();
  sets.forEach(s => s.forEach(t => allTypes.add(t)));
    ...
    <div className="flex items-center gap-2 flex-wrap">
      {platforms.map(p => {
        ...
        return (
          ...
          >
            <span style={{ color: isSelected ? PLATFORM_COLORS[p] : 'var(--muted-foreground)' }}>
              <Icon size={size * 0.5} />
            </span>
          </button>
        );
