...
import { mockProjects, STATUS_LABELS, WorkflowStatus } from '@/data/mockData';
            ...
            <div className="space-y-2.5">
              {mockProjects.map(p => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">{p.contentCount} conteúdos</span>
                </button>
              ))}
            </div>
          </div>
              ...
              {pendingApprovals.length === 0 ? (
                ...
                pendingApprovals.map(c => (
                  <div key={c.id} className="flex items-center justify-between p-3 rounded-lg bg-secondary">
                    ...
                    <span className="text-xs text-muted-foreground">{c.assignee.name.split(' ')[0]}</span>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="space-y-3">
            {contents.slice(0, 5).map(c => (
              <div key={c.id} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-xs text-muted-foreground ml-auto">{new Date(c.createdAt).toLocaleDateString('pt-BR')}</span>
              </div>
            ))}
          </div>
        </div>
