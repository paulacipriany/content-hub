...
const defaultIntegrations: Integration[] = [
  {
    key: 'instagram',
    name: 'Instagram',
    description: 'Publique fotos, carrosséis, reels e stories',
    icon: '📸',
    color: '#E4405F',
  ...
  },
  {
    key: 'facebook',
    name: 'Facebook',
    description: 'Gerencie postagens e interações na sua página',
    icon: '👤',
    color: '#1877F2',
  ...
  },
  {
    key: 'tiktok',
    name: 'TikTok',
    description: 'Publique e agende vídeos na plataforma',
    icon: '🎵',
    color: '#000000',
  ...
  },
  {
    key: 'linkedin',
    name: 'LinkedIn',
    description: 'Publique artigos e posts profissionais',
    icon: '💼',
    color: '#0A66C2',
  ...
  },
  {
    key: 'youtube',
    name: 'YouTube',
    description: 'Envie e gerencie vídeos no canal',
    icon: '▶️',
    color: '#FF0000',
  ...
  },
  {
    key: 'pinterest',
    name: 'Pinterest',
    description: 'Crie e agende pins nos seus paineéis',
    icon: '📌',
    color: '#BD081C',
...
const SettingsIntegrationsPage = () => {
  ...
  const handleConnect = (key: string) => {
    setIntegrations(prev =>
      prev.map(i =>
        i.key === key
          ? { ...i, status: 'connected' as const, account: `conta_${key}` }
          : i
      )
    );
  ...
  const handleDisconnect = (key: string) => {
    setIntegrations(prev =>
      prev.map(i =>
        i.key === key
          ? { ...i, status: 'disconnected' as const, account: undefined }
          : i
      )
    );
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-primary/5 border border-primary/15">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Conecte suas redes sociais para publicar e agendar conteúdos diretamente pela plataforma.
            </p>
          </div>
        </div>
          ...
          {integrations.map(integration => {
            ...
            return (
              ...
              >
                {/* Platform icon */}
                <div
                  className={`w-12 h-12 rounded-xl ${integration.bgColor} flex items-center justify-center flex-shrink-0 text-xl`}
                >
                  {integration.icon}
                </div>
